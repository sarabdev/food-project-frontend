          ...
          <table className="w-full min-w-[950px] text-left text-sm">
            ...
            <tbody className="divide-y">{shipments.map((shipment) => (
              <tr key={shipment.id} className="hover:bg-slate-50">
                ...
                <td className="px-5 py-4 text-right"><Link className="inline-flex items-center gap-2 font-semibold text-forest-700" to={`/shipments/${shipment.id}`}>Open <ArrowRight size={16} /></Link></td>
              </tr>
            ))}</tbody>
          </table>
        </div>
